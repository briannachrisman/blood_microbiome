...
def GetCoverages(sample_name, contig, hhv, flank):
    ...
    for r in reads:
        ...
        seqs1 = {k:[p[0] for p in pair_dict[k] if 'NC_' in k[0]]  for k in pair_dict}
        seqs2 = {k:[p[1] for p in pair_dict[k] if 'NC_' in k[1]]  for k in pair_dict}
    
    seqs1= {k:seqs1[k] for k in seqs1 if len(seqs1[k])>0}
    seqs2= {k:seqs2[k] for k in seqs2 if len(seqs2[k])>0}
    seqs = [v for vv in seqs1.values() for v in vv] + [v for vv in seqs2.values() for v in vv]
    if len(seqs)>0:
        print("Writing seqs...")
        with pysam.AlignmentFile(sample_dir + '.paired_aligned_to_hg38_herpes.sam', 'r') as samfile: 
            outfile = pysam.AlignmentFile("%s/%s_%s.bam" % (OUT_DIR, sample_name, hhv), "wb", template=samfile)
            for s in seqs: outfile.write(s)

for SAMPLE in df_herpesvirus[df_herpesvirus['Human betaherpesvirus 6A']>0].index: GetCoverages(SAMPLE, herpes_6a_contig, 'hhv6A')
for SAMPLE in df_herpesvirus[df_herpesvirus['Human betaherpesvirus 6B']>0].index: GetCoverages(SAMPLE, herpes_6b_contig, 'hhv6B')
for SAMPLE in df_herpesvirus[df_herpesvirus['Human betaherpesvirus 7']>0].index: GetCoverages(SAMPLE, herpes_7_contig, 'hhv7')